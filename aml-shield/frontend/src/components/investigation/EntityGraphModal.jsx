...
function deriveTimeline(data, node) {
  ...
  // Selected node is itself an alert or SAR — return it as a single-event
  // timeline so the "Showing selected event" hint reads correctly.
  if (node.type === 'CASE') return { alerts: [node], sars: [] };
  if (node.type === 'SAR')  return { alerts: [], sars: [node] };

  // Helper: every node id directly connected to a given id.
  ...
  // Customer node: alerts/SARs directly linked.
  ...
  // Counterparty node: hop to neighbour customers, then to their alerts/SARs.
