...
const fs = require('fs');
const { db, initSchema, seedAdminDataIfEmpty } = require('./db');
...
for (const s of sarRows) {
  ...
  for (let d = 1; d <= count; d++) {
    ...
    const body =
`AML SHIELD — SUPPORTING DOCUMENT
SAR ID:         ${s.sar_id}
Case ID:        ${s.case_id}
...
`;
    ...
    insertDoc.run(s.sar_id, filename, docType, path.join('uploads', diskName), size,
                  s.prepared_by, uploadedAt);
  }
}
...
for (const s of sarRows) {
  if (s.detection_date) {
    insertAudit.run(s.sar_id, 'Detection Logged', s.prepared_by, `${s.detection_date} 09:00:00`,
      `Detection from source alert ${s.source_alert_id}`);
  }
  if (s.draft_created_date) {
    insertAudit.run(s.sar_id, 'Draft Created', s.prepared_by, `${s.draft_created_date} 10:00:00`,
      `Draft SAR created; ${s.documents_count} supporting document(s) attached`);
  }
  if (s.reviewed_by) {
    insertAudit.run(s.sar_id, 'Submitted for Review', s.prepared_by, `${s.draft_created_date} 14:00:00`,
      `Routed to reviewer: ${s.reviewed_by}`);
  }
  if (s.filed_date) {
    insertAudit.run(s.sar_id, 'SAR Filed', s.approved_by || s.reviewed_by, `${s.filed_date} 11:30:00`,
      `Filed with ${s.reporting_jurisdiction}${s.regulator_reference ? ` (ref: ${s.regulator_reference})` : ''}`);
  }
  if (s.acknowledged_date) {
    insertAudit.run(s.sar_id, 'Regulator Acknowledged', s.approved_by || s.reviewed_by, `${s.acknowledged_date} 09:45:00`,
      `${s.reporting_jurisdiction} acknowledged filing`);
  }
  if (s.law_enforcement_hold && Number(s.law_enforcement_hold)) {
    insertAudit.run(s.sar_id, 'Legal Hold Applied', s.approved_by || s.reviewed_by, `${s.latest_activity_date} 12:00:00`,
  ...
  if (exportCount > 0 && s.last_exported_at) {
    ...
    insertAudit.run(s.sar_id, 'Export Package Generated', s.current_owner || s.prepared_by, when,
...
for (const s of sarRows) {
  ...
  for (let r = 0; r < n; r++) {
    ...
    insertRetrieval.run(s.sar_id, requester || 'system', purpose, when, when);
  }
}

// Customers / Accounts / Transactions (derived deterministically from alerts CSV)
...
for (const c of customerRows) {
  ...
  db.prepare('UPDATE accounts SET current_balance = ? WHERE account_number = ?')
    .run(running, account.account_number);
}

// Users + manager default settings (derived from freshly-loaded alerts / SAR rows above)
seedAdminDataIfEmpty();

const counts = {
  alerts:         db.prepare('SELECT COUNT(*) AS c FROM alerts').get().c,
  user_profiles:  db.prepare('SELECT COUNT(*) AS c FROM user_profiles').get().c,
  manager_settings: db.prepare('SELECT COUNT(*) AS c FROM manager_settings').get().c,
  sar_filings:    db.prepare('SELECT COUNT(*) AS c FROM sar_filings').get().c,
  cases:          db.prepare('SELECT COUNT(*) AS c FROM cases').get().c,
  documents:      db.prepare('SELECT COUNT(*) AS c FROM documents').get().c,
  audit_trail:    db.prepare('SELECT COUNT(*) AS c FROM audit_trail').get().c,
  retrieval_log:  db.prepare('SELECT COUNT(*) AS c FROM retrieval_log').get().c,
  ...
  transactions_alerted: db.prepare('SELECT COUNT(*) AS c FROM transactions WHERE is_alerted = 1').get().c
};
